...
Current status note: the user reported in web chat on 2026-07-01 that the smoke-test has already been run. The smoke-test result has not yet been recorded in this plan. Next action is to analyze that completed smoke-test run from its run URL or run id, not to launch another smoke-test blindly.
...
- The GitHub smoke-test result must be recorded before any 21000-second full run.
```

## Known risks to check in the completed smoke-test

The smart-search-14 engine is generated by extending the previous cover-stitch-cache generator. This is intentional, but it creates a risk that the run only renames modes and still behaves too much like smart-search-13. The completed smoke-test should therefore be checked not only for artifacts, but also for shard metadata and modes:
...
If the smoke-test merely reproduced old `mlct22-1c8736b46b59a730`-style candidates with no useful mode diversity, do not run the full 20-job search immediately. Inspect the generated C++ and strengthen the endpoint-feasible scoring first.
...
These were the prepared inputs. The user says this smoke-test has already been run.
...
Smoke is technically green if all shards compile, all produced shard JSONs pass the scaled checker, and artifact `rich-cover-stitch-run-summary` contains `smart_run_summary.json` and `smart_run_summary.md`.
...
Do not use these until the completed smoke-test is recorded and judged green/useful.
