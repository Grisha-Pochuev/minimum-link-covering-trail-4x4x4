...
- `22` links: open; best checked partial candidate is `62/64`.
...
- `frontier/active_run.json`: current operational run and do-not-duplicate state.
...
6. active workflow and its launch document
7. latest relevant `runs/` archive
8. candidate banks/originals
9. Actions jobs, logs and artifacts for the run being analyzed
...
Authoritative live record: `frontier/active_run.json`.
...
The active run uses an immutable historical commit. Do not change or rerun it merely to improve source organization.
...
- search-22: endpoint repair reached exact `62/64` before the full run.
...
4. Review the chat and improve memory/process without changing an active immutable run.
...
- record active run in `frontier/active_run.json`;
